...
const Navigation = ({ links, onDemoClick }: { links: any[], onDemoClick: () => void }) => {
  ...
  return (
    <nav className="fixed top-0 left-0 right-0 z-50 flex justify-between items-center px-8 py-3 glass-nav rounded-full mx-auto mt-6 max-w-6xl">
      <Link to="/" onClick={handleScrollToTop} className="font-serif italic text-2xl tracking-tight text-atelier-ink">Wealthy Vibes</Link>
      ...
      <button 
        onClick={onDemoClick}
        className="gilded-gradient px-6 py-2 rounded-full text-white font-medium text-sm tracking-wide hover:opacity-90 transition-opacity shadow-md whitespace-nowrap"
      >
        Book a Demo
      </button>
    </nav>
